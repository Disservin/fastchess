...
class DrawTracker {
   public:
    DrawTracker() noexcept {
        move_number_ = config::Tournament.get().draw.move_number;
        move_count_  = config::Tournament.get().draw.move_count;
        draw_score   = config::Tournament.get().draw.score;
    }
...
class ResignTracker {
   public:
    ResignTracker() noexcept {
        resign_score = config::Tournament.get().resign.score;
        move_count_  = config::Tournament.get().resign.move_count;
        twosided_    = config::Tournament.get().resign.twosided;
    }
...
class MaxMovesTracker {
   public:
    MaxMovesTracker() noexcept { move_count_ = config::Tournament.get().maxmoves.move_count; }
