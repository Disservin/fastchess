...
Gauntlet::Gauntlet(const stats_map& results) : BaseTournament(results) {
    const auto& config     = *config::TournamentConfig;
    const auto num_players = config::EngineConfigs->size();

    generator_ =
        std::make_unique<GauntletScheduler>(book_.get(), num_players, config.rounds, config.games, match_count_, config.gauntlet_seeds);
}

void Gauntlet::start() {
    LOG_TRACE("Starting gauntlet tournament...");
...
void Gauntlet::startNext() {
    ...
    pool_.enqueue(&Gauntlet::createMatch, this, match.value());
}

void Gauntlet::createMatch(const Scheduler::Pairing& pairing) {
...
bool Gauntlet::shouldPrintRatingInterval(std::size_t round_id) const noexcept {
    const auto& cfg = *config::TournamentConfig;
...
bool Gauntlet::shouldPrintScoreInterval() const noexcept {
    const auto& cfg = *config::TournamentConfig;
