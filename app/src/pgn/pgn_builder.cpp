...
PgnBuilder::PgnBuilder(const config::Pgn &pgn_config, const MatchData &match, std::size_t round_id, std::size_t game_id, int games)
    ...
    if (games == 1)
        ...
        addHeader("Round", std::to_string(round_id) + "." + std::to_string((game_id - 1) % 2));
