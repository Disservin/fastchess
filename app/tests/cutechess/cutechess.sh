...
cp fast-chess cutechess-testing
...
if [[ ! -f ./cutechess-cli ]]; then
   ...
   wget https://github.com/ppigazzini/static-cutechess-cli/releases/download/v1.3.1/cutechess-cli-linux-64bit.zip
...
if [[ ! -f ./UHO_4060_v2.epd ]]; then
   ...
   wget https://github.com/official-stockfish/books/raw/master/UHO_4060_v2.epd.zip
...
grep PlyCount fast-chess-out.pgn | grep -o "[0-9]*" | sort -n > fast-chess-out.plycount
diff ./cutechess-cli-out.plycount ./fast-chess-out.plycount
if [[ $? ]]; then
