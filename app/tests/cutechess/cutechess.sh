...
for binary in ./cutechess-cli ./mercury
do
...
diff ./cutechess-cli-out.finished ./mercury-out.finished
if [[ $? -ne 0 ]]; then
...
grep PlyCount mercury-out.pgn | grep -o "[0-9]*" | sort -n > mercury-out.plycount
diff ./cutechess-cli-out.plycount ./mercury-out.plycount
if [[ $? -ne 0 ]]; then
