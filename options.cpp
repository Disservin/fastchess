...
namespace CMD
{
    ...
    Options::Options(int argc, char const *argv[])
    {
        ...
        // When we find another '-command' command if it's "-engine" move on to another engine, otherwise go back to general cli
        // If(command == "-engine" goto engine stuff) else back to cli
