...
bool Board::makeMove(Move move) {
    ...
    if (moves.find(move) == -1) return false;
    ...
    if (enpassant_square_ != NO_SQ) hash_key_ ^= updateKeyEnPassant(enpassant_square_);
    ...
    if (capture != NONE) {
        half_move_clock_ = 0;
        if (typeOfPiece(capture) == ROOK) removecastling_rightsRook(to_sq);
...
void Board::unmakeMove(Move move) {
    ...
    if ((p == WHITEKING && capture == WHITEROOK) || (p == BLACKKING && capture == BLACKROOK)) {
        ...
        if (capture != NONE) placePiece(capture, to_sq);
...
Bitboard Board::allBB() const { return us(WHITE) | us(BLACK); }

Square Board::KingSQ(Color c) const { return lsb(pieces(KING, c)); }

uint64_t Board::getHash() const { return hash_key_; }

Bitboard Board::pieces(PieceType type, Color color) const { return pieceBB_[color][type]; }

Piece Board::pieceAt(Square square) const { return board_[square]; }
...
Color Board::getSideToMove() const { return side_to_move_; }

Square Board::getEnpassantSquare() const { return enpassant_square_; }

uint8_t Board::getCastlingRights() const { return castling_rights_; }

Bitboard Board::getSquaresBetweenBB(int sq_1, int sq_2) const {
    return squares_between_bb_[sq_1][sq_2];
}

bool Board::isChess960() const { return chess960_; }
...
GameResult Board::isGameOver() {
    ...
    if (count == 3) {
        if (pieces<BISHOP, WHITE>() || pieces<BISHOP, BLACK>()) return GameResult::DRAW;
        if (pieces<KNIGHT, WHITE>() || pieces<KNIGHT, BLACK>()) return GameResult::DRAW;
    }
...
bool Board::isSquareAttacked(Color c, Square sq) const {
    ...
    if ((pieces(BISHOP, c) | pieces(QUEEN, c)) & BishopAttacks(sq, all)) return true;
    if ((pieces(ROOK, c) | pieces(QUEEN, c)) & RookAttacks(sq, all)) return true;
...
File Board::squareFile(Square sq) { return File(sq & 7); }

Rank Board::squareRank(Square sq) { return Rank(sq >> 3); }

Square Board::fileRankSquare(File f, Rank r) { return Square((r << 3) + f); }
...
uint8_t Board::diagonalOf(Square sq) { return 7 + squareRank(sq) - squareFile(sq); }

// returns anti diagonal of given square
uint8_t Board::anti_diagonalOf(Square sq) { return squareRank(sq) + squareFile(sq); }

bool Board::sameColor(int sq1, int sq2) { return ((9 * (sq1 ^ sq2)) & 8) == 0; }

Color Board::colorOf(Piece p) {
    if (p == NONE) return NO_COLOR;
    return Color(p / 6);
}

Color Board::colorOf(Square loc) const { return Color((pieceAt(loc) / 6)); }
...
uint64_t Board::updateKeyEnPassant(Square sq) const { return RANDOM_ARRAY[772 + squareFile(sq)]; }

uint64_t Board::updateKeyCastling() const { return castlingKey[castling_rights_]; }

uint64_t Board::updateKeyside_to_move() const { return RANDOM_ARRAY[780]; }

Board::Board() { initializeLookupTables(); }
...
std::string MoveToRep(Board &b, Move move, bool isLan) {
    ...
    std::string rep;
    if (pt != PAWN) rep = repPieceType[pt];
    ...
    if (move.promotion_piece != NONETYPE) rep += "=" + repPieceType[move.promotion_piece];
    ...
    if (moves.size == 0 && inCheck) return rep + "#";

    if (inCheck) return rep + "+";
