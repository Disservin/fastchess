...
struct Move
{
    ...
    Move(Square from_sq_cp, Square to_sq_cp, PieceType _moving_piece, PieceType promotion_piece_cp)
        : from_sq(from_sq_cp), to_sq(to_sq_cp), moving_piece(_moving_piece),
          promotion_piece(promotion_piece_cp)
    {
    }
};
...
{
    os << "from sq " << int(m.from_sq) << " to_sq " << int(m.to_sq) << " promotion "
       << int(m.promotion_piece);
    return os;
}
