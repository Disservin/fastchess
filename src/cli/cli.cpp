...
TimeControl::Limits parseTc(const std::string &tcString) {
    ...
    const bool has_colon              = str_utils::contains(tcString, ":");
    ...
    if (has_colon) {
        ...
        if (colon.size() == 2) {
            int64_t minutes = static_cast<int64_t>(std::stod(remainingStringVector[0]) * 1000);
            int64_t seconds = static_cast<int64_t>(std::stod(remainingStringVector[1]) * 1000);
            tc.time = minutes * 60 + seconds;
        } else {
            int64_t hours = static_cast<int64_t>(std::stod(remainingStringVector[0]) * 1000);
            int64_t minutes = static_cast<int64_t>(std::stod(remainingStringVector[1]) * 1000);
            int64_t seconds = static_cast<int64_t>(std::stod(remainingStringVector[2]) * 1000);
