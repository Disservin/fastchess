...
double EloPentanomial::score(const Stats& stats) noexcept {
...
double EloPentanomial::nEloError(const Stats& stats) noexcept {
    const double variance = variance(stats);
...
double EloPentanomial::diff(const Stats& stats) noexcept {
    return scoreToEloDiff(score(stats));
}
...
std::string EloPentanomial::los(const Stats& stats) const noexcept {
    const double los = (1 - std::erf(-(score(stats) - 0.5) / std::sqrt(2.0 * variancePerGame(stats)))) / 2.0;
    std::stringstream ss;
