...
TEST_SUITE("Openings") {
    TEST_CASE("check epd openings") {
        ...
        std::vector<std::string> pgn = book.getEpdBook();

        CHECK(pgn.size() == 10);
        CHECK(pgn.capacity() == 10);
        CHECK(pgn[0] == "1n1qkb1r/rp3ppp/p1p1pn2/2PpN2b/3PP3/1QN5/PP3PPP/R1B1KB1R w KQk - 0 9");
        CHECK(pgn[9] == "rn1qkb1r/4pp1p/3p1np1/2pP4/4P3/2N5/PP3PPP/R1BQ1KNR w kq - 0 9");
    }
